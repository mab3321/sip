...
func (p *mediaPortPipeline) init(
	...
	if err != nil {
		return fmt.Errorf("failed to setup pipeline session: %w", err)
	}

	err = p.setupInput(mc, audioToRoom, dtmfToRoom)
	if err != nil {
		return fmt.Errorf("failed to setup pipeline input: %w", err)
	}
	err = p.setupOutput(mc, incomingSampleRate)
	if err != nil {
		return fmt.Errorf("failed to setup pipeline output: %w", err)
	}
	return nil
}
